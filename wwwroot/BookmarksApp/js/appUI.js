...
const BOOKMARKS_REFRESH_INTERVAL = 5000;
let bookmarksRefreshHandle = null;
let lastKnownBookmarksEtag = null;

Init_UI();

function startBookmarksAutoRefresh() {
    if (bookmarksRefreshHandle) return;
    bookmarksRefreshHandle = setInterval(async () => {
        const etag = await Bookmarks_API.Head();
        if (etag && etag !== lastKnownBookmarksEtag) {
            lastKnownBookmarksEtag = etag;
            renderBookmarks(true);
        }
    }, BOOKMARKS_REFRESH_INTERVAL);
}
function stopBookmarksAutoRefresh() {
    if (bookmarksRefreshHandle) {
        clearInterval(bookmarksRefreshHandle);
        bookmarksRefreshHandle = null;
    }
}
...
async function renderBookmarks(autoRefresh = false) {
    if (!autoRefresh) {
        stopBookmarksAutoRefresh();
        showWaitingGif();
    }
    ...
    const previousScrollTop = autoRefresh ? contentElement.scrollTop() : 0;
    ...
    if (Bookmarks !== null) {
        ...
        if (autoRefresh) {
            contentElement.scrollTop(previousScrollTop);
        } else {
    ...
    if (Bookmarks_API.Etag) {
        lastKnownBookmarksEtag = Bookmarks_API.Etag;
    }
    if (!autoRefresh) {
        startBookmarksAutoRefresh();
    }
}
